...
export function StateCalculator({ initialStateCode }: StateCalculatorProps) {
  ...
  const estimate = useMemo(() => {
    ...
    if (pvwattsKwh != null) {
      ...
      for (let y = 0; y < 20; y++) {
        const yrSavings =
          pvwattsKwh * ratePerKwh * Math.pow(1 + annualEscalator, y);
      ...
      return {
        ...
        capacityFactor:
          theoreticalMaxKwh > 0 ? pvwattsKwh / theoreticalMaxKwh : 0,
      };
    }
  ...
  return (
    <main className="flex flex-1 flex-col items-center bg-white">
      ...
      <footer className="w-full py-6 text-center text-sm text-zinc-400 border-t border-zinc-100">
        <p>
          Estimates are approximate. Actual production depends on panel
